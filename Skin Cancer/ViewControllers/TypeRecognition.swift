...
class TypeRecognition: UIViewController, UINavigationControllerDelegate, UIImagePickerControllerDelegate {
...
//        detectScene(image: ciImage)
    ...
    func imagePickerController(_ picker: UIImagePickerController, didFinishPickingMediaWithInfo info: [String : Any]) {
        ...
        guard let image = info[UIImagePickerControllerOriginalImage] as? UIImage else {
            fatalError("Can't pick up image from UIImagePickerController")
        }
        scene.image = image
...
//
//        guard let model = try? VNCoreMLModel(for: TypeCancer().model) else {
